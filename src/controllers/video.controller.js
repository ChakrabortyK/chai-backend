...
const publishAVideo = asyncHandler(async (req, res) => {
    ...
    const videoFile = await uploadCloud(videoFileLocalPath);
    const thumbnail = await uploadCloud(thumbnailFileLocalPath);
...
const updateVideo = asyncHandler(async (req, res) => {
    ...
    if (thumbnailFile) {
        await deleteCloud(previousVideo.thumbnail?.public_id);

        // upload new one
        thumbnailUploadOnCloudinary = await uploadCloud(thumbnailFile);
...
const deleteVideo = asyncHandler(async (req, res) => {
    ...
    if (video.videoFile) {
        await deleteCloud(video.videoFile.public_id, "video");
    }

    if (video.thumbnail) {
        await deleteCloud(video.thumbnail.public_id);
    }
